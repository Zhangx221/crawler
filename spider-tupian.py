...
#  1爬取并解析图片的路径，直接保存图片
def save1File():
	saveDir = "./movie_poster/"  # 保存电影海报路径，方便修改
	img_src = "https://img2.doubanio.com/view/photo/s_ratio_poster/public/p480747492.jpg"
	path_list = img_src.split("/")  # 截取图片名称
	# print(saveDir+path_list[-1])
	objpath = saveDir+path_list[-1]  # 拼接图片存储的完整路径
...
def saveFiles(dbpath):
	...
	conn = sqlite3.connect(dbpath)
	cur = conn.cursor()
	# 如果未来想要将图片应用在网页上，建议使用id作为海报名称，方便网址拼接
